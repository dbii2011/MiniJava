        ...
        if (okCollectAndPartialResolve) {
            ...
            if (okCompleteResolve) {
                ...
                if (okCheckType) {
					...
					// 1. Le code du Main en premier (point d'entr̩e)
					f.append(this.main.getCode(factory));
					
					// 2. On arr̻te le programme une fois le main termin̩
					f.add(factory.createHalt());
					
					// 3. On place le code des classes en dessous (elles seront appel̩es plus tard)
					for (ClassDeclaration c : this.classes) {
